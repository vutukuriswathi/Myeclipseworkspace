...
public class AbcMeTest extends TestBase {
	...
	public void checkShowScreenPage() throws InterruptedException {
		...
		if (TestBase.platform.equalsIgnoreCase("ios")) {
			...
			util.waitUntillElementIsVisible(showCollectionScreenPage.iosSeriesTitle);
	...
	public void checkChangeNamePage() throws InterruptedException {
		...
		if (TestBase.platform.equalsIgnoreCase("ios")) {
			util.waitUntillElementIsVisible(profilePage.iosAvatar);
			profilePage.iosAvatar.click();
			util.waitUntillElementIsVisible(profilePage.iosChangeName);
			profilePage.iosChangeName.click();
			TestBase.takeScreenshot("changeNamePage");

			// verify name your avatar text
			try {
				Assert.assertTrue(profilePage.iosNameYourAvatarTxt.getText().equals(Constants.nameAvatarTxt),
						"pick an avatar actual text is" + profilePage.iosNameYourAvatarTxt.getText()
								+ "but expected text is" + Constants.nameAvatarTxt);

			} catch (Exception e) {
				Reporter.log("pick an avatar actual text is" + profilePage.iosNameYourAvatarTxt.getText()
						+ "but expected text is" + Constants.nameAvatarTxt);
				ErrorUtil.addVerificationFailure(e);
			}
	...
	public void checkShowToAddToMyShows() throws InterruptedException {
		...
		if (TestBase.platform.equalsIgnoreCase("ios")) {
			...
			try {
				Assert.assertTrue(showCollectionScreenPage.iosShowTitle.getText().equals(showTitle),
	...
	public void checkCollectionToAddToMyShows() throws InterruptedException {
		...
		if (TestBase.platform.equalsIgnoreCase("ios")) {
			...
			try {
				Assert.assertTrue(showCollectionScreenPage.iosShowTitle.getText().equals(showTitle),
						"show is not added to my shows.Actual show is "
								+ showCollectionScreenPage.iosShowTitle.getText() + "expected text is" + showTitle);
			} catch (Exception e) {
				Reporter.log("show is not added to my shows");
				ErrorUtil.addVerificationFailure(e);
