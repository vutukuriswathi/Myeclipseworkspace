...
public class Common {
	...
	public void w3cValidatorTest(String url) throws InterruptedException {
		...
		Assert.assertFalse(url + "has " + result, Integer.parseInt(errors[0]) > 0 || Integer.parseInt(errors[2]) > 0);
	...
	public void moreNavigation() {
		...
		more.click();
		WebElement world_item = getWhenVisible(world_menuitem, 10);
		for (int i = 8; i <= 16; i++) {
	...
	// it is visible
	public WebElement getWhenVisible(WebElement locator, int timeout) {
