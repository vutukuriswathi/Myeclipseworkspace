...
public class HomePage {
	...
	// TopStories menu
	@AndroidFindBy(name = "Top Stories. Double Tap to Open")
	public MobileElement topStoriesMenuItem;

	// TopStoriescategoryheading
	@AndroidFindBy(className = "android.widget.TextView")
	public List<MobileElement> topStoriesHeading;
	...
	public void openApp() {
		...
		if (TestBase.platform.equalsIgnoreCase("android")) {
			try {
				...
				wait.until(ExpectedConditions.visibilityOf(onBoarding_next));
			} catch (Exception e) {
				e.printStackTrace();
			}
			try {
	...
	public void verifyTopStoriesHeading() {
		...
		if (TestBase.platform.equalsIgnoreCase("android")) {
			Assert.assertTrue(topStoriesHeading.get(0).getText().equals(Constants.topStoriesHeading),
